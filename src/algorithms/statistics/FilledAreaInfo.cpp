...
void FilledAreaInfo::apply(QString&, image::Image& image) {
    ...
    fields.emplace_back("Column", pf::Int(0, int(image.size().width - 1), 0));
    fields.emplace_back("Row", pf::Int(0, int(image.size().height - 1), 0));
    ...
    start_coords.x = std::get<int>(values->at("Column"));
    start_coords.y = std::get<int>(values->at("Row"));

    auto info = getAreaInfo(image, start_coords, rule);

    QMessageBox::information(nullptr, "Filled area stats", QString("Area: %1 px").arg(info.area_px));
}
